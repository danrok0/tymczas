...
class Objective:
    ...
    reward_money: int = 0
    reward_satisfaction: int = 0
    reward_description: str = ""
    time_limit: Optional[int] = None  # Limit tur (None = bez limitu)
    turns_remaining: Optional[int] = None
    prerequisites: List[str] = None  # Lista ID celów, które muszą być ukończone
...
class ObjectiveManager:
    ...
    def _initialize_objectives(self):
        ...
        # CELE ŚREDNIE (Poziom 2) - zwiększone wymagania
        self.add_objective(Objective(
            id="growing_city",
            ...
            prerequisites=["first_population"],
        ...
        # CELE ZAAWANSOWANE (Poziom 3) - oznacz jako LOCKED
        self.add_objective(Objective(
            id="metropolis",
            title="Metropolia",
            ...
            prerequisites=["growing_city", "happy_citizens"],
            status=ObjectiveStatus.LOCKED
        ))
