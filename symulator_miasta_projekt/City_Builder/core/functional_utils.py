...
def safe_map(func: Callable[[T], U], iterable: Iterable[T]) -> List[U]:
    ...
    Args:
        ...
        iterable: Iterable do przetworzenia
...
def safe_reduce(func: Callable[[T, T], T], iterable: Iterable[T], initial: Optional[T] = None) -> Optional[T]:
    ...
    Args:
        func: Funkcja redukcji
        iterable: Iterable do zredukowania
        initial: Wartość początkowa
        
    Returns:
        Zredukowana wartość lub None w przypadku błędu
    """
    try:
...
def building_generator(buildings: List[Any]) -> Generator[Any, None, None]:
    """
    Generator budynków z filtrowaniem.
    
    Args:
        buildings: Lista budynków
        
    Yields:
        Budynki jeden po drugim
    """
...
def event_sequence_generator(events: List[Dict], 
    ...
    Args:
        ...
        repeat: Czy powtarzać sekwencję
        
    Yields:
        Wydarzenia w sekwencji
    """
    if repeat:
        event_cycle = cycle(events)
        while True:
            yield next(event_cycle)
    else:
        for event in events:
            yield event
...
def population_growth_generator(initial_population: int, 
    ...
    Args:
        ...
        growth_rate: Współczynnik wzrostu
        max_steps: Maksymalna liczba kroków
        
    Yields:
        Tuple (krok, populacja)
    """
    population = initial_population
    
    for step in range(max_steps):
        yield (step, int(population))
        population *= (1 + growth_rate)
        
        # Ograniczenie wzrostu
        if population > 1000000:  # Maksymalna populacja
            break

def batch_generator(iterable: Iterable[T], batch_size: int) -> Generator[List[T], None, None]:
    """
    Generator dzielący iterable na partie.
    
    Args:
        iterable: Iterable do podziału
...
def performance_monitor(func: Callable) -> Callable:
    ...
    Args:
        func: Funkcja do monitorowania
        
    Returns:
        Funkcja z monitorowaniem wydajności
    """
    @wraps(func)
